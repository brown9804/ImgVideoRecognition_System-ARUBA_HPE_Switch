...
for f1 in files:
	...
	if len(location_port[0]) > 0:
		...
		for i in sorted(location_port[0]):
			if i not in port_x:
				port_x.append(i)
		...
		for j in sorted(location_port[1]):
			if j not in port_y:
				port_y.append(j)
		#### Before filtered y coordinate
		...
		for pix_port in port_fil:
			...
			cv2.rectangle(img, pix_port, (pix_port[0] + w_port, pix_port[1] + h_port), (0,255,255), 5)
			#### Label on drawing
			cv2.putText(img, 'Port ', pix_port, cv2.FONT_HERSHEY_TRIPLEX, 1, (0, 255, 255), 5)
		...
		metic = (w_img-h_img)/count_port

	loc_full_port = sorted(list(OrderedDict.fromkeys(loc_full_port)))
